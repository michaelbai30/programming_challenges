...
def calc_collatz_cycle_length(num, collatz_cyc_len_dict):
    ...
    if num not in collatz_cyc_len_dict:
        if num % 2 == 1:
            # recursive call (3n + 1)
            cyc_len = calc_collatz_cycle_length(3*num + 1, collatz_cyc_len_dict)
            collatz_cyc_len_dict[num] = cyc_len + 1
            return collatz_cyc_len_dict[num]
        else:
            # recursive call (n // 2)
            cyc_len = calc_collatz_cycle_length(num//2, collatz_cyc_len_dict)
            collatz_cyc_len_dict[num] = cyc_len + 1
            return collatz_cyc_len_dict[num]

    # MEMOIZATION
    # if num already in dict, we don't need to recompute it, just return the value.      
    else:
        return collatz_cyc_len_dict[num]
...
def get_max_cycle_len(begin, end, collatz_cyc_len_dict):
    max_cycle_len = 0
    while begin <= end:
        cycle_len = calc_collatz_cycle_length(begin, collatz_cyc_len_dict)
...
for line in sys.stdin:
    elements=line.split()
    begin_seq = int(elements[0])
    end_seq = int(elements[1])
    num_arr.append([begin_seq, end_seq])
